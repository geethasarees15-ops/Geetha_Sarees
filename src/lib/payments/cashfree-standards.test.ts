...
describe("cashfree-standards", () => {
  it("builds canonical Cashfree callback URLs", () => {
    expect(buildCashfreeReturnUrl("https://geethasarees.vercel.app/")).toBe(
      "https://geethasarees.vercel.app/api/cashfree/redirect?order_id={order_id}",
    );
    expect(buildCashfreeNotifyUrl("https://geethasarees.vercel.app")).toBe(
      "https://geethasarees.vercel.app/api/cashfree/webhook",
    );
  });
  ...
  it("adds whitelisting guidance to domain errors", () => {
    const message = readCashfreeCheckoutError(
      ...
      { whitelistOrigin: "https://geethasarees.vercel.app" },
    );
