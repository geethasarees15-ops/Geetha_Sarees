...
export const POOLER_SESSION_EXHAUSTED_MESSAGE =
...
export function isSessionPoolExhausted(error: unknown): boolean {
  const haystack = collectErrorText(error).toLowerCase();
  return (
    haystack.includes("emaxconnsession") ||
    haystack.includes("max clients reached in session mode")
  );
}
...
export function mapProductSaveError(error: unknown): Error {
  ...
  if (isSessionPoolExhausted(error)) {
    return new Error(POOLER_SESSION_EXHAUSTED_MESSAGE);
  }
