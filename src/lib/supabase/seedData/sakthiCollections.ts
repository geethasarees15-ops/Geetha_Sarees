...
/** Geetha Sarees saree categories (homepage carousel + /collections/[slug]) */
...
function collectionCopy(label: string) {
  return {
    title: label,
    description: `Explore our ${label} at Geetha Sarees — premium sarees for every occasion.`,
  };
}
...
export default async function seedSakthiCollections(
  ...
  for (let i = 0; i < SAKTHI_COLLECTION_LABELS.length; i++) {
    ...
    const imageAlt = `${label} — Tamil Nadu saree model, Geetha Sarees`;
